    ...
    return (
      <div className="relative grid min-h-screen place-items-center overflow-hidden px-4 py-10">
        ...
        <div className="glass-panel relative z-10 w-full max-w-md rounded-3xl p-6 text-center sm:p-8">
          <h2 className="mb-2 text-2xl font-bold text-slate-900 dark:text-slate-100">{t('auth.reset.invalidLink')}</h2>
          <p className="mb-4 text-slate-500 dark:text-slate-300">{t('auth.reset.noToken')}</p>
          <Link to="/forgot-password" className="btn-primary rounded-xl px-5 py-2.5">{t('auth.reset.requestNewLink')}</Link>
        </div>
      </div>
    );
  }
        ...
        <div className="glass-panel rounded-3xl p-6 sm:p-8">
          {success ? (
            <div className="space-y-4 text-center">
              ...
              <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('auth.reset.passwordUpdated')}</h2>
              <p className="text-slate-500 dark:text-slate-300">
                {t('auth.reset.passwordUpdatedMessage')}
              </p>
            ...
            <>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">{t('auth.reset.title')}</h2>
              <p className="mb-6 mt-1 text-sm text-slate-500 dark:text-slate-300">{t('auth.reset.description')}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="newPassword" className="label">{t('auth.reset.newPassword')}</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder={t('auth.reset.atLeastEight')}
                      required
                      minLength={8}
                    />
                    <button
                ...
                <div>
                  <label htmlFor="confirm" className="label">{t('auth.reset.confirmNewPassword')}</label>
                  <div className="relative">
                    <Lock className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
                    <input
                      id="confirm"
