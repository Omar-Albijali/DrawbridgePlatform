...
import { InventoryItem, InventoryStatus, ScheduleType, CreateInventoryItemRequest } from '../types';
...
const Inventory: React.FC = () => {
    ...
    useEffect(() => {
        const fetchInventory = async () => {
            if (user?.id) {
                try {
                    // If user is retailer, fetch their inventory
    ...
    const toggleAutoRestock = async (id: string) => {
    ...
    const openConfigModal = (item: InventoryItem) => {
        setSelectedItem(item);
        setIsModalOpen(true);
    };

    const handleSaveConfig = async (config: Partial<import('../types').AutoOrderConfigDTO>) => {
        ...
        try {
            ...
            // Refresh inventory to get the latest state
            // Optimization: could just update local state
    ...
    const handleAddInventory = async (request: CreateInventoryItemRequest) => {
        try {
    ...
    const outOfStockCount = inventory.filter(i => i.status === InventoryStatus.OUT_OF_STOCK).length;
    const autoRestockEnabled = inventory.filter(i => i.autoRestock).length;
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredInventory.map((item) => (
                                <tr
                                    key={item.id}
                                    className={`hover:bg-gray-50 transition-colors ${item.status === InventoryStatus.LOW_STOCK ? 'bg-amber-50/50' :
                                        item.status === InventoryStatus.OUT_OF_STOCK ? 'bg-red-50/50' : ''
                                        }`}
                                >
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className={`font-semibold ${item.currentStock === 0 ? 'text-red-600' :
                                            item.currentStock < (item.autoOrderConfig?.minThreshold || 0) ? 'text-amber-600' : 'text-navy-800'
                                            }`}>
                                            {item.currentStock}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-center text-navy-600">
                                        {item.autoOrderConfig?.minThreshold || 'N/A'}
                                    </td>
                                    <td className="px-6 py-4 text-center">
                                        <button
                                        ...
                                        >
                                            <span
                                                className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${item.autoRestock ? 'left-7' : 'left-1'
                                                    }`}
                                            />
                                        </button>
                                    </td>
