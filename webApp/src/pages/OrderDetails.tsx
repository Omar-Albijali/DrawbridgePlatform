...
const OrderDetails: React.FC = () => {
    ...
    useEffect(() => {
        const fetchOrder = async () => {
            if (id) {
                try {
                    const data = await orderService.getById(id.toString());
                    setOrder(data);
                } catch (error) {
                ...
                <div>
                    ...
                    <div className="flex items-center gap-2 text-navy-500 mt-1">
                        ...
                        <span className="bg-gray-100 px-2 py-0.5 rounded text-sm font-medium text-navy-700 capitalize">
                            {((order.status as any).name || order.status).toLowerCase().replace('_', ' ')}
                        </span>
                    </div>
                </div>
                        ...
                        <div className="max-w-[100vw] overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tfoot className="bg-gray-50">
                                    <tr>
                                        <td colSpan={3} className="px-6 py-4 text-right font-medium text-navy-600">Subtotal</td>
                                        <td className="px-6 py-4 text-right font-bold text-navy-800">
                    ...
                    <div className="bg-white rounded-xl shadow-card p-6">
                        ...
                        <div className="space-y-4">
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-sm font-medium text-navy-800">{((order.shippingMethod as any)?.name || order.shippingMethod || 'Standard').toLowerCase().replace('_', ' ')}</p>
                                    {order.trackingNumber && (
                                        <p className="text-xs text-navy-500 mt-1">
                                            Tracking: <a href={order.trackingUrl || '#'} className="text-primary-600 hover:underline">{order.trackingNumber}</a>
                                        </p>
                                    )}
                                </div>
                            </div>
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    <p className="text-sm text-navy-600">
                                        Shipping Address usually goes here (fetch from User/Profile)
                                    </p>
                                </div>
                            </div>
                        </div>
